...
object YandexMarkerIconFactory {
    ...
    fun createClusterIcon(context: Context, count: Int, config: ClusterConfig?): Bitmap {
        ...
        val backgroundColor =
                config?.backgroundColor?.let { ColorUtils.fromColorValue(it) }
                        ?: Color.parseColor("#007AFF")
        val textColor = config?.textColor?.let { ColorUtils.fromColorValue(it) } ?: Color.WHITE
        val borderColor = config?.borderColor?.let { ColorUtils.fromColorValue(it) } ?: Color.WHITE
